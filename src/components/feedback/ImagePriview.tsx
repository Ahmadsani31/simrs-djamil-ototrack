...
export default function ImagePriview({ uriImage, index }: any) {
  ...
  return (
    <View key={index}>
      <TouchableHighlight onPress={onPress} className="rounded-lg bg-gray-300 p-1 ">
        <View className="flex-row items-center">
          <AntDesign name={nameIcon ? 'up-circle' : 'down-circle'} size={14} color="black" />
          <Text className="ms-5 text-center">show image</Text>
        </View>
      </TouchableHighlight>
      <View className="w-full flex-1">
        <AccordionItem isExpanded={open} viewKey="Accordion">
          <Image className="aspect-[3/4] w-full rounded-lg" source={{ uri: uriImage }} />
        </AccordionItem>
      </View>
    </View>
  );
